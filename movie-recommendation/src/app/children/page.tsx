          ...
          <section key={index} id={cat.id} className="px-6 md:px-16">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
              {cat.movies.map((movie) => (
                <div
                  key={movie.id}
                  className="group relative aspect-[3/4] rounded-2xl overflow-hidden cursor-pointer border-4 border-transparent hover:border-pink-500 transition-all duration-300 transform hover:-translate-y-2 shadow-xl hover:shadow-pink-500/30"
                >
                  <Image
                    src={movie.image}
                    alt="Movie Cover"
